...
set_logger_name("desktop_server")

import context
from base_client import BaseClient
from db.pg_model import PGModel
from db.constants import DB_VDI
from constants import SERVER_TYPE_VDI_SERVER, DEFAULT_LICENSE
...
from api.limit.api_limit import load_api_limit
from api.user.user import build_global_admin_info
################################################################################

def check_license(ctx):

    ctx.license = ctx.pgm.load_license_info()
    if not ctx.license:
        logger.error("load license failed, use tryout license.")
        ctx.license = DEFAULT_LICENSE
        return False

    (y,m,d) = str.split(ctx.license["terminate_date"], '-')
    ...
    logger.info("license=%s" % ctx.license)
...
class DesktopService():
    ...
    def start(self):
        ...
        # since desktop is the centrol point, its connect pool shall be large enough
        ...
        ctx.api_limits = load_api_limit(ctx)
        ctx.admin_users = build_global_admin_info(ctx)
        ...
        peer_server = BaseServer(get_listening_url(SERVER_TYPE_VDI_SERVER), 1, handler)
        ...
        time.sleep(1)
        logger.info("vdi server is running now.")
...
def _get_opt_parser():
    ...
    MSG_USAGE = "vdi_server [-c <conf_file>]"

...
from constants import (
    ...
    SPICE_CONNECT_FILE_BASE_URI,
    SPICE_CONNECT_FILE_DIR_NUMBERS,
)
...
def _clean_connection_file(file_name, base_path = SPICE_CONNECT_FILE_DIR):
...
def _get_dir_index():
    ...
    index = 0
    for _, dirnames, _ in os.walk(SPICE_CONNECT_FILE_DIR):
        break
...
def create_spice_connect_files(desktop_broker):
    ''' create spice connect files '''
    ctx = context.instance()
    if not desktop_broker:
        logger.error("desktop_brokers is none.")
        return False
    
    if not os.path.isdir(SPICE_CONNECT_FILE_DIR):
        os.system("mkdir -p %s" % SPICE_CONNECT_FILE_DIR)
        os.system("chmod 777 %s" % SPICE_CONNECT_FILE_DIR)
    ...
    for resource_id, broker in desktop_broker.items():
        ...
        if not isinstance(broker_port, list) or len(broker_port)< 2:
            continue
        ...
        path = "%s/%s" % (SPICE_CONNECT_FILE_DIR, desktop_id_prefix)
        ...
        try:
            with open(conn_file_path, "w") as f:
                contents = ["[virt-viewer]\n"]
                contents.append("type=spice\n")
                contents.append("host=%s\n" % broker["broker_host"])
                contents.append("port=%s\n" % str(broker_port[0]))
                contents.append("tls-port=%s\n" % str(broker_port[1]))
                contents.append("password=%s\n" % broker["graphics_passwd"])
                ...
                if broker.get("usb_redirect", 1) == 1:
                    ...
                    #contents.append("redirect-on-connect=" + _get_usb_filter(broker))
                contents.append("enable-smartcard=0\n")
        ...
        broker["connect_file_uri"] = "%s/%s/%s" % (SPICE_CONNECT_FILE_BASE_URI, desktop_id_prefix, conn_file)

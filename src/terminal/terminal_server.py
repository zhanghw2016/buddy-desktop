...
set_logger_name("terminal_server")

import context
from base_client import BaseClient
from db.pg_model import PGModel
from db.constants import DB_VDI
from constants import SERVER_TYPE_VDI_TERMINAL_SERVER,VDI_TERMINAL_PULL_SERVER_PORT,VDI_TERMINAL_SOCKET_SERVER_PORT,HOST_SERVICE_ADDRESS
...
from esocket import EsocketServer
from terminal_agent_handler import TerminalAgentServiceHandler
...
class TerminalService():
    ...
    def __init__(self, conf):
        ...
        # initialize terminal_context
        ctx = context.instance()
        # ctx.pull_url = "inproc://pullserver_%d" % id(self)
        ctx.pull_url = "tcp://127.0.0.1:%d" % VDI_TERMINAL_PULL_SERVER_PORT
    ...
    def start(self):
        ...
        logger.info("starting service handler ...peer_server")
        handler = ServiceHandler()
        peer_server = BaseServer(get_listening_url(SERVER_TYPE_VDI_TERMINAL_SERVER), 1, handler)
        ...
        logger.info("starting pull service handler ...pull_server")
        ...
        # start terminal agent socket server to handler terminal request
        handler = TerminalAgentServiceHandler(thread_num = 5)
        terminal_socket = EsocketServer(ctx,host=HOST_SERVICE_ADDRESS,
                                       port=VDI_TERMINAL_SOCKET_SERVER_PORT,
                                       listen_size=100,
                                       timeout=3,
                                       handler=handler)
        terminal_thr = threading.Thread(target=terminal_socket.start, args=())
        terminal_thr.setDaemon(True)
        terminal_thr.start()
        self.thrs.append(terminal_thr)

        # wait while servers are actually started
        time.sleep(1)
        logger.info("terminal server is running now.")
...
def _get_opt_parser():
    ...
    MSG_USAGE = "terminal_server [-c <conf_file>]"
